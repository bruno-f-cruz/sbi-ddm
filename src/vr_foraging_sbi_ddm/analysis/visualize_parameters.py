...
def plot_parameter_grid_2d(
    ...
    simulator = JaxPatchForagingDdm(max_sites_per_window=window_sites)
...
def plot_all_parameter_combinations(
    ...
    param_names = ["drift_rate", "reward_bump", "failure_bump", "noise_std"]
    param_ranges = {name: (float(prior_low[i]), float(prior_high[i])) for i, name in enumerate(param_names)}

    # Fixed parameter values (use medians)
    fixed_values = {name: float(prior_median[i]) for i, name in enumerate(param_names)}
    ...
    plot_parameter_grid_2d(
        ...
        fixed_params={"failure_bump": fixed_values["failure_bump"], "noise_std": fixed_values["noise_std"]},
        grid_size=grid_size,
    ...
    plot_parameter_grid_2d(
        ...
        fixed_params={"reward_bump": fixed_values["reward_bump"], "noise_std": fixed_values["noise_std"]},
        grid_size=grid_size,
        window_sites=window_sites,
    ...
    plot_parameter_grid_2d(
        ...
        fixed_params={"reward_bump": fixed_values["reward_bump"], "failure_bump": fixed_values["failure_bump"]},
        grid_size=grid_size,
        window_sites=window_sites,
    ...
    plot_parameter_grid_2d(
        ...
        param2_range=param_ranges["failure_bump"],
        fixed_params={"drift_rate": fixed_values["drift_rate"], "noise_std": fixed_values["noise_std"]},
        grid_size=grid_size,
        window_sites=window_sites,
    ...
    plot_parameter_grid_2d(
        ...
        param1_range=param_ranges["reward_bump"],
        param2_range=param_ranges["noise_std"],
        fixed_params={"drift_rate": fixed_values["drift_rate"], "failure_bump": fixed_values["failure_bump"]},
        grid_size=grid_size,
        window_sites=window_sites,
    ...
    plot_parameter_grid_2d(
        ...
        param2_range=param_ranges["noise_std"],
        fixed_params={"drift_rate": fixed_values["drift_rate"], "reward_bump": fixed_values["reward_bump"]},
        grid_size=grid_size,
        window_sites=window_sites,
...
def plot_parameter_effect_summary(window_sites: int = 100, save_dir: str = "parameter_visualizations"):
    """
    ...
    for param_idx, (param_name, param_range) in enumerate(zip(param_names, param_ranges)):
        ...
        for i, param_val in enumerate(param_values):
            ...
            # Plot
            ax = axes[param_idx, i]
            plot_single_window(ax, window, theta, title=f"{param_name}={param_val:.3f}")

    fig.suptitle("Effect of Each Parameter on Behavior (4D Theta)", fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
...
def plot_noise_comparison(window_sites: int = 100, save_dir: str = "parameter_visualizations"):
    """
    ...
    for i, noise_std in enumerate(noise_levels):
        ...
        # Plot
        ax = axes[i]
        plot_single_window(ax, window, theta, title=f"Noise σ = {noise_std:.3f}")

    fig.suptitle("Effect of Noise on Behavioral Trajectories", fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "summary":
        ...
        print("Generated:")
        print("  - parameter_effects_summary.png")
        ...
        print("\nVisualization complete!")
        print("Generated 6 parameter grids + 2 summary plots:")
        print("  - grid_drift_vs_reward.png")
        print("  - grid_drift_vs_failure.png")
        print("  - grid_drift_vs_noise.png")
        print("  - grid_reward_vs_failure.png")
        print("  - grid_reward_vs_noise.png")
        print("  - grid_failure_vs_noise.png")
        print("  - parameter_effects_summary.png")
        print("  - noise_comparison.png")
