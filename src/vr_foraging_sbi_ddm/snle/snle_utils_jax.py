...
def compare_snle_vs_simulator(simulator, true_theta, posterior_samples, num_patches=100, rng_key=None, save_path=None):
    """
...
def save_model(snle_params, y_mean, y_std, mode="multi", base_dir="snle_models"):
    ...
    model_dict = {"params": snle_params, "mode": mode, "timestamp": timestamp, "y_mean": y_mean, "y_std": y_std}
...
def load_model(model_path):
    ...
    dict with keys:
        ...
        - simulator : JaxPatchForagingDdm (Simulator instance)
...
def get_model_directory(config: Config, make_dir=False):
    """
