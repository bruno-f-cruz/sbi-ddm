...
def compare_all_models(base_output_dir, n_tests=100):
    ...
    model_dirs = [d for d in base_dir.iterdir() if d.is_dir() and (d / "model.pkl").exists()]
    ...
    for model_dir in sorted(model_dirs)[:1]:
        ...
        try:
            summary_stats, config = evaluate_single_model(model_dir, n_tests=n_tests, save_results=True)
            ...
            row["calibration_score"] = sum(summary_stats[p]["well_calibrated"] for p in param_names) / len(param_names)
    ...
    if "n_features" in df.columns and df["n_features"].notna().any():
        ...
        print("=" * 80)
        feature_analysis = df.groupby("n_features").agg({"calibration_score": ["mean", "std", "count"]}).round(3)
...
def main():
    ...
    comparison_df = compare_all_models(BASE_OUTPUT_DIR, n_tests=N_SBC_TESTS)
